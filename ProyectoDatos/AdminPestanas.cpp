...
void AdminPestanas::ExplorarHistorialPestanas() {
	...
	while (bandera) {
		// Limpiar pantalla y mostrar el men� de administraci�n de pestanas
		cout << "\x1B[2J\x1B[H";
		...
		while (true) {
			...
			if (GetAsyncKeyState('N') & 0x8000) {
				...
				Pestana* pes = new Pestana(nombrePestana);
				InsertarPrimero(pes);
				nodoActual = tail; // Se mueve autom�ticamente a la nueva pestana
...
int AdminPestanas::obtenerOpcion()
{
	...
	// Si la entrada no es un n�mero entero
	while (cin.fail()) {
		cin.clear();  // Limpia el estado de error de cin
		cin.ignore(1000, '\n');  // Ignora hasta 1000 caracteres o hasta un salto de l�nea
		cout << "Entrada invalida. Por favor, ingrese un numero: ";
		cin >> op;
	}

	return op;  // Retorna la opci�n v�lida
}
